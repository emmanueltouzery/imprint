...
testShouldFail :: Spec
testShouldFail = it "fails parsing a bad string" $
    assertBool "fails" $ isLeft $ parseFormat "%ss"

testShouldFail2 :: Spec
testShouldFail2 = it "fails parsing a bad string" $
